...
export default function Consultation() {
  ...
  const validateStep = (step) => {
    ...
    if (step === 2) {
      if (!formData.budgetRange) newErrors.budgetRange = 'Budget range required';
      if (!formData.metalPreference) newErrors.metalPreference = 'Metal preference required';
    }
    setErrors(newErrors);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Book your free consultation for a custom black spinel engagement ring. In-person in Olympia or nationwide via Zoom." />
      ...
      <div className="min-h-screen bg-ink-black">
        ...
        <main className="pt-32 pb-space-10">
          <div className="section-narrow">
            ...
            {/* Actual Form */}
            <form onSubmit={handleSubmit} className="bg-parchment-dark metallic-border heavy-shadow p-space-8">
              {/* Step 1: Contact */}
              {currentStep === 1 && (
                <motion.div
                  key="step1"
                ...
                >
                  <div>
                    ...
                    />
                    {errors.fullName && <p className="text-rust text-sm mt-space-1">{errors.fullName}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {errors.email && <p className="text-rust text-sm mt-space-1">{errors.email}</p>}
                  </div>

                  <div>
                    <label htmlFor="phone" className="form-label">
                      Phone <span className="text-sepia-shadow">(optional)</span>
                    </label>
                    <input
                      type="tel"
                      id="phone"
                  ...
                  <div>
                    <label className="form-label">Consultation Type <span className="text-aged-gold">*</span></label>
                    <div className="grid grid-cols-2 gap-space-4">
                      {[
                        { value: 'in-person', label: 'In-Person', sublabel: 'Olympia, WA' },
                        { value: 'zoom', label: 'Zoom', sublabel: 'Nationwide' },
                      ].map((option) => (
                        <button
                          key={option.value}
                          type="button"
                          onClick={() => updateField('consultationType', option.value)}
                        ...
                        >
                          <span className="block font-spectral text-base text-bone-white uppercase tracking-wide">{option.label}</span>
                          <span className="font-crimson text-sm text-charcoal-mist">{option.sublabel}</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              {currentStep === 2 && (
                ...
                >
                  <div>
                    ...
                    {errors.budgetRange && <p className="text-rust text-sm mt-space-1">{errors.budgetRange}</p>}
                  </div>

                  <div>
                    <label className="form-label">Metal Preference <span className="text-aged-gold">*</span></label>
                    <div className="grid grid-cols-2 gap-space-3">
                      {['14kt Gold', '18kt Gold', 'Platinum', 'Palladium', 'Unsure'].map((metal) => (
                        <button
                          key={metal}
                          type="button"
                    ...
                    {errors.metalPreference && <p className="text-rust text-sm mt-space-1">{errors.metalPreference}</p>}
                  </div>

                  <div className="grid grid-cols-2 gap-space-4">
                    <div>
                      <label htmlFor="fingerSize" className="form-label">Finger Size</label>
                      <input
                        type="text"
                        id="fingerSize"
                    ...
                    <div>
                      <label htmlFor="whichFinger" className="form-label">Which Finger</label>
                      <select
                        id="whichFinger"
              ...
              {currentStep === 3 && (
                ...
                >
                  <div>
                    <label htmlFor="vision" className="form-label">Tell Us Your Vision</label>
                    <textarea
                      id="vision"
              ...
              {errors.submit && <p className="text-rust text-sm text-center mt-space-4">{errors.submit}</p>}

              {/* Navigation */}
              <div className="flex justify-between mt-space-8 pt-space-6 border-t border-obsidian">
                {currentStep > 1 ? (
                  <button type="button" onClick={prevStep} className="btn-secondary flex items-center gap-space-2">
                ...
                {currentStep < totalSteps ? (
                  <button type="button" onClick={nextStep} className="btn-primary flex items-center gap-space-2">
                    Next Step
                    <ChevronRight className="w-4 h-4" />
                  </button>
                ) : (
                  <button type="submit" disabled={isSubmitting} className="btn-primary flex items-center gap-space-2">
                    {isSubmitting ? (
                      <>
