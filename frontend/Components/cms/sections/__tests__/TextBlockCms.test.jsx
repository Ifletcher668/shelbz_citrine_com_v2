...
const { heritageMarked } = require('@/lib/marked-extensions');

describe('TextBlockCms', () => {
  beforeEach(() => {
    heritageMarked.parse.mockReturnValue('<p>Parsed body</p>');
  });
  ...
  it('calls heritageMarked.parse with the body string', () => {
    const body = '# My heading';
    render(<TextBlockCms data={makeTextBlock({ body })} />);
    expect(heritageMarked.parse).toHaveBeenCalledWith(body);
  });
  ...
  it('renders nothing for body when body is empty/null', () => {
    heritageMarked.parse.mockReturnValue(null);
  ...
  it('adds prose-heritage class when enable_prose is true', () => {
    const { container } = render(<TextBlockCms data={makeTextBlock({ enable_prose: true })} />);
    expect(container.querySelector('.prose-heritage')).toBeInTheDocument();
  });

  it('omits prose-heritage class when enable_prose is false', () => {
    const { container } = render(<TextBlockCms data={makeTextBlock({ enable_prose: false })} />);
  ...
  it('forwards texture variant and opacity to Section when texture is set', () => {
    ...
    expect(() =>
      render(<TextBlockCms data={makeTextBlock({ texture: 'metal', texture_opacity: 50 })} />)
    ).not.toThrow();
  });

  it('passes texture={false} to Section when texture is null', () => {
    // No texture — Section renders without BackgroundTexture.
    const { container } = render(<TextBlockCms data={makeTextBlock({ texture: null })} />);
    expect(container.querySelector('section')).toBeInTheDocument();
  });

  it('passes texture={false} to Section when texture is "none"', () => {
    const { container } = render(<TextBlockCms data={makeTextBlock({ texture: 'none' })} />);
    expect(container.querySelector('section')).toBeInTheDocument();
  });
